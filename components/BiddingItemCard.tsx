      ...
      <div className="w-full md:w-72 h-56 md:h-auto overflow-hidden bg-slate-800 flex-shrink-0 relative">
        ...
        {isExpired && (
          <div className="absolute inset-0 bg-slate-950/80 flex items-center justify-center backdrop-blur-md">
            <span className="bg-white text-slate-950 px-6 py-2 rounded-2xl font-black text-xs tracking-widest uppercase shadow-2xl">
              Auction Concluded
            </span>
          </div>
        )}
      ...
      <div className="flex-1 p-8 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="font-black text-2xl text-white tracking-tight mb-1">{item.name}</h3>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Asset ID: {item.id}</p>
            </div>
          ...
          <div className="flex flex-wrap gap-8 mt-6">
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Current Evaluation</span>
              <span className="text-3xl font-black text-indigo-400 tracking-tighter">${item.highestBidAmount.toLocaleString()}</span>
            </div>
            
            <div className="w-[1px] bg-white/5 hidden sm:block"></div>
            
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Time To Expiry</span>
              <span className={`text-xl font-bold ${isExpired ? 'text-rose-500' : 'text-amber-500 font-mono tracking-tight'}`}>
                {timeLeft}
              </span>
            </div>
          </div>
        ...
        <div className="mt-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6 pt-6 border-t border-white/5">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0">
              <span className="text-[10px] text-slate-500 font-black uppercase tracking-widest block leading-none mb-1.5">
                ...
                {tiedBidders.length > 0 ? (
                  tiedBidders.map((name, idx) => (
                    <div key={name} className="flex items-center gap-2">
                      ...
                      <span className="text-indigo-500 font-black text-[11px]">${item.highestBidAmount.toLocaleString()}</span>
                      {idx < tiedBidders.length - 1 && <span className="text-slate-700">•</span>}
                    </div>
                  ))
                ) : (
                  <span className="font-bold text-slate-600 italic">No market activity yet</span>
                )}
                
                {item.isTie && (
                  <span className="px-2.5 py-0.5 rounded-lg bg-rose-500/20 text-rose-400 text-[10px] font-black uppercase tracking-tighter ml-1">
          ...
          <button className={`px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex-shrink-0 ${
            isExpired 
